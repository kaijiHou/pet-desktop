...
Updated for V2: Chinese UI labels, semantic animation names, single-image
character system. Pocket UI tests moved to test_pocket_window_gui.py (the
real V2 PocketWindow), so this file no longer references the obsolete
pocket_ui.PocketDialog.
"""
...
class TestGuiConstruction:
    ...
    def test_settings_data_log_buttons_connected(self, pet_window):
        d = pet_window_mod.SettingsDialog(pet_window.config, pet_window)
        assert d.open_data_button is not None
        assert d.open_log_button is not None
        d.close()
    ...
    def test_reminder_list_shows_and_deletes_pending_item(self, pet_window):
        pet_window.reminder.add_reminder("GUI smoke", datetime.now() + timedelta(hours=1))
        dialog = ReminderListDialog(pet_window.reminder, pet_window)
        assert dialog.reminder_list.count() == 2  # group header + item
