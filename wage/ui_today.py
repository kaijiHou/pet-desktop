...
class TodayWageWindow(QWidget):
    def __init__(self, service, parent=None):
        ...
        self.amount = QLabel("未配置"); self.amount.setStyleSheet(f"font-size: 22pt; font-weight: 700; color: {theme.ACCENT};"); self.detail = QLabel(""); self.detail.setWordWrap(True); self.progress = QLabel(""); self.clock_btn = QPushButton("下班打卡"); self.clock_btn.setObjectName("primary"); self.clock_btn.clicked.connect(self._clock_out); lay.addWidget(self.amount); lay.addWidget(self.detail); lay.addWidget(self.progress); lay.addWidget(self.clock_btn); root.addWidget(card)
    ...
    def refresh(self):
        if not self.service.configured:
            self.amount.setText("未配置"); self.detail.setText("请先设置工资与工作时间"); self.progress.clear(); self.clock_btn.setEnabled(False); return
        ...
        if self.service.settings.privacy_mode:
            self.amount.setText(f"今日进度 {snap.progress}%"); self.detail.setText(f"距 17:30 的工作进度 · 已加班 {snap.overtime_minutes // 60}h{snap.overtime_minutes % 60:02d}m"); self.progress.clear()
        else:
